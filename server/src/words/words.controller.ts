...
export class WordsController {
  ...
  @ApiResponse({ status: 201, type: CreateWordDto, description: 'Слово успішно створено' })
  @ApiResponse({ status: 409, description: 'Слово вже існує' })
  @Post()
  async createWord(@GetCurrentUserId() userId, @Body(ValidationPipe) createWordDTO: CreateWordDto) {
    return this.wordsService.createWord(userId, createWordDTO);
  }

  @ApiOperation({ summary: 'Отримання слів побуквено' })
  @ApiResponse({ status: 200, type: WordEntity, description: 'Список знайдених слів'  })
  @Get('search')
  ...
  getWordsByUser(@GetCurrentUserId() userId, @Query() paginationDto: PaginationDto) {
  ...
  @ApiResponse({ status: 200, type: UpdateWordDTO, description: 'Слово успішно оновлено' })
  @ApiResponse({ status: 404, description: 'Слово не знайдено або немає доступу' })
  @Patch(':id')
  updateWord(
